...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onClick, onInteraction, onBuy, lang = 'en' }) => {
  ...
  return (
    <div 
        ...
        className="group bg-white dark:bg-slate-800 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100 dark:border-slate-700 flex flex-col overflow-hidden relative select-none cursor-pointer"
    >
      
      {/* Static Image - Forced white background for product visibility */}
      ...
      <div className="p-2.5 flex flex-col flex-grow">
        ...
        {/* Name */}
        <div className="mb-2">
            <h3 className="text-gray-900 dark:text-gray-100 font-medium text-xs sm:text-sm leading-snug line-clamp-2 min-h-[2.5em]" title={product.name}>
            {product.name}
            </h3>
        </div>
        ...
        <div className="flex items-center gap-3 mb-2" onClick={(e) => e.stopPropagation()}>
            ...
            <button 
                onClick={handleLike}
                className="flex items-center gap-1 group/btn p-1 -ml-1 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-full transition-all"
            >
                ...
                <span className={`text-[10px] font-bold leading-none ${likeState === 'liked' ? 'text-red-500' : 'text-gray-400 dark:text-gray-500'}`}>
            ...
            <button 
                ...
                className="p-1 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-full transition-transform active:scale-90"
            >
